...
class ADSBExchange extends Source{
    ...
    getLocation = async (icao:string) =>{
        /// <reference lib="dom" />
        const response = await fetch(`https://globe.adsbexchange.com/data/traces/${icao.slice(-2)}/trace_recent_${icao}.json`, {
            "headers": {
        ...
        let [time_offset,lat,lon, alt_baro, ground_speed, ground_track] = last_trace;
    ...
    getLocationFull = async (icao:string) =>{
        /// <reference lib="dom" />
        const response = await fetch(`https://globe.adsbexchange.com/data/traces/${icao.slice(-2)}/trace_full_${icao}.json`, {
            "headers": {
              "accept": "application/json, text/javascript, */*; q=0.01",
              "accept-language": "de-DE,de;q=0.9",
              "sec-ch-ua": "\"Chromium\";v=\"116\", \"Not)A;Brand\";v=\"24\", \"Brave\";v=\"116\"",
        ...
        console.log(trace);
        let last_trace = trace[trace.length-1]
        let [time_offset,lat,lon, alt_baro, ground_speed,unknown2,unknown3,unknown4,unknown5] = last_trace;
        ...
        console.log(time_offset,lat,lon, alt_baro, ground_speed,unknown2,unknown3,unknown4,unknown5)
    }
}
